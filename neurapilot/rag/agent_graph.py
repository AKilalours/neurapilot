...
_FLASH_KW = re.compile(
    r'\b(flashcard|flash\s*card|make\s+.{0,8}card|cards\s+on|flip\s+card)\b', re.I)
...
def build_pipeline(
    ...
    def classify_node(state: AgentState) -> AgentState:
        ...
        # ── Extract ONLY the current request (strip context prefix) ─────────
        # _ctx_query wraps messages in "[Prior conversation...]...Current request: X"
        text = full_text
        if "Current request (answer THIS" in full_text:
        ...
        try:
            ...
            valid_intents = {"ask", "summarize", "flashcards", "quiz", "plan"}
            alias = {
    ...
    for node in ["ask","summarize","flashcards","quiz","plan"]:
        g.add_edge(node, END)
