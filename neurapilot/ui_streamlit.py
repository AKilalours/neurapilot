...
# ─────────────────────────────────────────────────────────────────────────────
st.set_page_config(
    page_title="NeuraPilot AI", page_icon="🧠",
...
def _get_py_ns() -> dict:
    ...
    All code blocks share the SAME namespace so variables defined in
    Step 1 are available in Step 2, Step 3, etc — just like a Jupyter notebook.
    """
    if "_py_ns" not in st.session_state:
        ns: dict = {}
        try:
            exec(
            ...
            pass
        st.session_state._py_ns = ns
    return st.session_state._py_ns
...
def _run_py(code: str) -> None:
    ...
    if eb.getvalue():
        # Show a friendlier error with a hint
        err = eb.getvalue()
        hint = ""
        if "NameError" in err:
            ...
            if var:
                hint = (f"\n\n💡 **Tip:** `{var.group(1)}` was not found. "
                        "Make sure you ran the earlier code block(s) above first "
                        "by clicking **▶ Run Python** on each step in order.")
        st.error(f"```\n{err}```{hint}")
...
def _ctx_query(text: str) -> str:
    """Inject recent turns as context — safely strips quiz/flashcard JSON to avoid intent poisoning."""
    recent = [m for m in st.session_state.msgs[-10:]
              if m.get("content") not in ("__FLASHCARDS__", "__QUIZ__", "__IMGBLOCK__", "")
              and m.get("content")]
    if len(recent) < 2:
    ...
    for m in recent[:-1]:
        ...
        content = m["content"]
        # Skip long AI outputs (quiz JSON / flashcard JSON) — they poison the classifier
        if m["role"] == "assistant" and len(content) > 400:
            content = content[:200] + "… [response truncated]"
        # Skip raw JSON blobs entirely
        if content.strip().startswith("[{") or content.strip().startswith('{"questions'):
            continue
        hist_parts.append(f"{role}: {content[:250]}")
    ...
    hist = "\n".join(hist_parts)
    return (
        f"[Prior conversation for context only — the CURRENT request is independent]\n"
        f"{hist}\n"
        f"[End context]\n\n"
        f"Current request (answer THIS, not the prior conversation): {text}"
    )
...
def _thinking_anim(placeholder: Any, step: int, intent: str = "ask") -> None:
    steps = [
        ...
        {"quiz": "Generating MCQ quiz", "flashcards": "Generating flashcards",
         "plan": "Building study plan", "summarize": "Summarizing content"}.get(intent, "Generating answer"),
        "Complete ✓",
    ]
...
def _run_agent(text: str, cid: str, think_ph: Any = None) -> dict:
    """Run the RAG pipeline with optional live thinking animation."""
    ...
    if re.search(r'\b(quiz|mcq|test me)\b', text, re.I):   intent_hint = "quiz"
        ...
        # For visualization requests, generate viz code and run it inline
        intent_hint = "ask"  # still route through agent for explanation first
...
def _render_fc(cards: list, topic: str, kpfx: str) -> None:
    if not cards:
        st.warning("No flashcards generated. Disable Strict mode and try again.")
        return
    st.markdown(f"**🃏 {len(cards)} Flashcards — {topic}**")
    for i, c in enumerate(cards):
        q    = c.get("q", ""); a = c.get("a", "")
        diff  = c.get("difficulty", "medium"); bloom = c.get("bloom_level", "remember")
        dcls  = {"easy": "fc-easy", "medium": "fc-medium", "hard": "fc-hard"}.get(diff, "fc-medium")
        st.markdown(
            f'<div class="fc-q">'
            f'<span class="fc-badge {dcls}">{diff}</span>'
            f'<span class="fc-badge fc-bloom">{bloom}</span>'
            f'<br><br>{q}</div>',
            unsafe_allow_html=True,
        )
        with st.expander("👁 Reveal Answer"):
            st.markdown(f'<div class="fc-a">{a}</div>', unsafe_allow_html=True)
            if c.get("citations"): st.caption("Sources: " + ", ".join(c["citations"]))
            _voice(a, f"{kpfx}_fc_{i}")
    st.success(f"✅ {len(cards)} cards saved → Flashcards tab")
...
def _render_msg_body(content: str, mkey: str) -> None:
    """Render message content — plain text or code blocks with optional Run button."""
    if "```" not in content:
        st.markdown(content)
        return
...
""", unsafe_allow_html=True)
    ...
    course_id = st.selectbox("Course", list(courses.keys()),
        ...
        with ca2:
            if st.button("✅ Create", use_container_width=True, key="nc_ok2"):
                ...
                if cid2 and cid2.replace("-","").replace("_","").isalnum():
                    cs2 = load_courses(S); cs2[cid2] = {"title": nc_title2.strip(), "description": ""}
                    save_courses(S, cs2); course_upload_dir(S, cid2)
                    dbmod.upsert_course(DB, cid2, nc_title2.strip())
...
with tab_chat:
    ...
    if user_input:
        ...
        if st.session_state.img_queue:
            ...
            think_ph = st.empty()
            # FIX 1: run agent AND save result BEFORE st.rerun()
            with st.spinner(""):
                result = _run_agent(user_input, course_id, think_ph)
...
with tab_anal:
    ...
    if not ixns:
        ...
        a3.metric("Faithfulness",  f"{df['faithfulness'].dropna().mean():.0%}" if df["faithfulness"].notna().any() else "—")
        a4.metric("Relevance",     f"{df['answer_relevance'].dropna().mean():.0%}" if df["answer_relevance"].notna().any() else "—")
        st.divider()
        t1, t2 = st.tabs(["📈 Performance", "🔍 Breakdown"])
        with t1:
            df2 = df.copy()
            df2["t"] = pd.to_datetime(df2["ts"], unit="s").dt.strftime("%H:%M")
            st.markdown("**Response Latency (ms)**")
            st.line_chart(df2.set_index("t")["latency_ms"].iloc[::-1])
            ec = [c for c in ["faithfulness","answer_relevance","context_precision"] if c in df.columns]
            if any(df[c].notna().any() for c in ec):
                st.markdown("**Quality Scores Over Time**")
                st.caption("Computed for `ask` and `summarize` only")
                st.line_chart(df[ec].dropna(how="all").iloc[::-1])
        with t2:
            col_i, col_tp = st.columns(2)
            with col_i:
                st.markdown("**Intent Breakdown**")
                if "intent" in df.columns: st.bar_chart(df["intent"].value_counts())
            with col_tp:
                st.markdown("**Top Topics**")
                if "topic" in df.columns: st.bar_chart(df["topic"].value_counts().head(8))
        with st.expander("📋 Full Interaction Log"):
            ec2 = [c for c in ["faithfulness","answer_relevance","context_precision"] if c in df.columns]
            log = df[["ts","intent","topic","latency_ms"]+ec2].copy()
            log["ts"] = pd.to_datetime(log["ts"],unit="s").dt.strftime("%Y-%m-%d %H:%M:%S")
            for c in ec2: log[c] = log[c].apply(lambda x: f"{x:.2f}" if x is not None else "—")
